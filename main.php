 ...
 * Description: Simplify your website users' login/registration journey. An easy to use plugin for implementing AJAX login and registration on your website. Beautiful front end interface and controls including styling, page redirection and registration user level.
...
/**
 * Adds styles and scripts of the plugin on front end
 */
...
/**
 * Adds styles and scripts of the plugin on back end
 */
function alw_admin_enqueue_scripts_and_styles( ){
	$plugin_dir_url = plugin_dir_url( __FILE__ );
	//css
...
add_action( 'admin_enqueue_scripts', 'alw_admin_enqueue_scripts_and_styles' );
...
function alw_print_scripts( ){
	...
	if ( preg_match( '~MSIE|Internet Explorer~i', $http_user_agent ) || strpos( sanitize_text_field( $http_user_agent ), 'Trident/7.0; rv:11.0') !== false ){
		?>
		<style>
